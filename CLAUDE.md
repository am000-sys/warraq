...
- **الإضافي المدفوع (مساعد المستند الذكي):** `Ask Document` و`Generate Report` و`تحسين دقّة القراءة` و`الترجمة` و`الفهرسة الذكيّة` (أعلام/أماكن/مصطلحات/موضوعات) — **مدعومة بـ Mistral chat (سريعة)**، مع Claude كبديل للأساسيّة. ليست ضمن الباقة الأساسيّة، وتُستدعى عند طلب المستخدم فقط.
