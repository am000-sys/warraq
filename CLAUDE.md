...
- **الإضافي المدفوع (مساعد المستند الذكي):** `Ask Document` و`Generate Report` و`تحسين دقّة القراءة` — **مدعومة بـ Mistral chat (سريعة)**، مع Claude كبديل. ليست ضمن الباقة الأساسيّة، وتُستدعى عند طلب المستخدم فقط.
