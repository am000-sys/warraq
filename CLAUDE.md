...
- **`kimi-*` / `moonshot-*`** ⇒ مزوّد Kimi (`src/lib/kimi.ts`) عبر **الدردشة المتوافقة
  ...
  - **تقسيم الإخراج إلى مقاطع** (`KIMI_CHUNK_TOKENS`، افتراضه ١٦٣٨٤): النداء متزامن
    ...
    في `study-poll.ts` من نقطة التوقّف. وبادئة الرسائل ثابتة فيُصيب كلّ مقطعٍ الكاشَ
    ولا تتضاعف كلفة إعادة إرسال الكتاب.
    - `study-poll.ts` **يَصِل المقاطع داخل النداء الواحد** ما دام الوقت يسمح
      (`STUDY_POLL_BUDGET_MS` و`MAX_ROUNDS`) بدل جولة استطلاع لكلّ مقطع. وما لم
  ...
  - **ميزانيّة زمنيّة للنداء** (`KIMI_TIMEOUT_MS`، افتراضه ١٢٠ ثانية مقابل
    `maxDuration = 300`): بلا سقفٍ نقطعه بأنفسنا، تقتل بيئة التشغيل الدالّةَ فلا
