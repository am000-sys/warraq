...
export async function POST(
  ...
  try {
    ...
    // تجاوز المهلة: النداء المتزامن لم يكتمل ضمن ميزانيّة الدالّة. إعادة المحاولة
    // بنفس الحجم والعمق ستتجاوزها كذلك، فالرسالة تدلّ على ما يُقصّر الزمن فعلاً.
    const isTimeout = /CHAT_TIMEOUT/.test(raw);
    ...
    const message = isTimeout
      ? "استغرق النموذج أطول من المهلة المتاحة فلم يكتمل الملخّص. جرّب عمقاً أقلّ (مكثّف بدل موسّع)، أو قسّم المادّة ولخّص كلّ جزء على حدة. لم يُخصم من رصيدك شيء."
      : isOverflow
    ...
    return NextResponse.json(
      { error: message },
      { status: isTimeout ? 504 : isOverflow ? 413 : isContentFlag ? 422 : isQuota ? 503 : 500 },
    );
  }
}
